...
export async function findAll(limit: number, skip: number) {
  return db.select().from(usersTable).limit(limit).offset(skip).orderBy(desc(usersTable.name));
}
...
export async function update(id: string, name: string) {
  return db.update(usersTable).set({ name }).where(eq(usersTable.id, id)).returning();
}
